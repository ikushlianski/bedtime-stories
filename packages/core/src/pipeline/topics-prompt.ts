...
export function buildTopicsBlock(items: EligibleTopic[], mode: 'auto' | 'manual' = 'auto'): string {
  ...
  const selectionRules = mode === 'manual'
    ? [
      '- Родитель уже осознанно выбрал именно ЭТИ темы для этой конкретной истории — вплети в сюжет КАЖДУЮ из них, не отбрасывай ни одной.',
      '- Тема должна раскрываться через события и поведение персонажей, а не проговариваться вслух и не превращаться в мораль. Сплети темы в единый сюжет, не перечисляй их по очереди и не уделяй им искусственно равное время.',
    ]
    : [
  ...
  return [
    ...
    ...selectionRules,
    '- В САМОМ КОНЦЕ ответа выведи отдельной строкой: «ТЕМЫ: <id выбранных тем через запятую или слово нет>». Если в ответе есть и строка «ФРАГМЕНТЫ», и эта строка — ТЕМЫ идёт сразу следующей строкой сразу после ФРАГМЕНТЫ, и вместе они и есть последние строки ответа. Эта строка служебная.',
    '---\n',
  ].join('\n')
}
