...
describe('telegram /new flow', () => {
  beforeEach(() => {
    ...
    vi.mocked(peekPendingAction).mockResolvedValue(null)
    vi.mocked(appendPendingSeedText).mockResolvedValue('')
  ...
  it('accumulates a message instead of firing the pipeline when a pending action exists', async () => {
    ...
    expect(vi.mocked(appendPendingSeedText)).toHaveBeenCalledWith(CHAT_ID, null, '42')
  ...
  it('accumulates a second message on top of the first', async () => {
    ...
    expect(vi.mocked(appendPendingSeedText)).toHaveBeenCalledWith(CHAT_ID, 'первое сообщение', 'второе сообщение')
  })
