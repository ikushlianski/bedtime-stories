...
- Cloud Run API service identity (`apiSa`) gets a bucket-scoped IAM binding (`roles/storage.objectAdmin` on `bedtime-prod-storage` only, not project-wide) — it currently has zero storage permission.
- The bucket gets a second IAM binding granting public read (`roles/storage.objectViewer` for `allUsers`), so generated portraits and uploaded references are servable as plain HTTPS URLs without a signing step, and so the same URLs can be re-sent to OpenRouter as image references for later generations without the API re-uploading or re-encoding anything.
...
- **Open question, not yet confirmed by the product owner:** the public-read grant covers the entire bucket, including whatever a person uploads as a character's own baseline reference image — in this app that could plausibly be a real photo of a real family member, not just AI-generated art. A generated portrait being public is low-stakes; an uploaded photo becoming a permanent, unauthenticated, crawlable URL is a materially different, hard-to-reverse call. This plan defaults to treating both the same way (bucket-wide public read) for simplicity, but that default has not been explicitly confirmed against this specific risk — see `spec.md`'s Decisions section and `todo.md`.
